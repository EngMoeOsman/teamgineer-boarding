...
Social links are rendered on the `/about` page and in the footer based on the array configured in the `social` object. You can set custom icons to each, but don't forget to import them in `src/once-ui/icons.ts`. Read the Teamgineer documentation](https://once-ui.com/docs/icons) for more information.
