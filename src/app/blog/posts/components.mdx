...
Magic Portfolio will replace some default HTML elements to Teamgineer components to integrate better in the design and add additional functionality. Furthermore, many common Teamgineer components are already imported and available for use.
...
The code block component works based on Teamgineer's CodeBlock. You can access it with the complete syntax for full customization, or by using the standard markdown syntax (triple backticks) with pre-configured props.
...
<Row fillWidth bottomRadius="l" padding="4" border="neutral-medium" background="surface">
    <Button href="https://once-ui.com/docs/codeBlock" weight="default" label="View on Teamgineer" size="s" variant="tertiary" suffixIcon="chevronRight"/>
</Row>
</Column>

### InlineCode

The `InlineCode` is another Teamgineer component that you can access either through the complete syntax or the short syntax (backticks). Usually the standard markdown syntax is recommended, since it doesn't have that much configuration options.
...
<Row fillWidth bottomRadius="l" padding="4" border="neutral-medium" background="surface">
    <Button href="https://once-ui.com/docs/inlineCode" weight="default" label="View on Teamgineer" size="s" variant="tertiary" suffixIcon="chevronRight"/>
</Row>
</Column>
...
Icons are used to visually represent actions, objects, or concepts. For instructions on importing new icons, head over to [the Teamgineer documentation](https://once-ui.com/docs/icons).
...
Since Magic Portfolio is based on Teamgineer, you can use all the components provided by Teamgineer in the `.mdx` files, but you need to import them first in the `src/product/mdx.tsx` file and pass it to the `components` object.
